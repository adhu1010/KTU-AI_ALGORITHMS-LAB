...
def best_first_search(start_board, goal_board, heuristic):
...
def read_heuristic():
    print("Choose a heuristic function (1 or 2):")
    ...
    if choice == 1:
        return lambda board, goal: sum([1 if board[i] != goal[i] and board[i] != 0 else 0 for i in range(9)])
    elif choice == 2:
        return lambda board, goal: sum(abs((board.index(i) // 3) - (goal.index(i) // 3)) + abs((board.index(i) % 3) - (goal.index(i) % 3)) for i in range(1, 9))
    else:
        print("Invalid choice! Using default heuristic (Misplaced Tiles).")
        return lambda board, goal: sum([1 if board[i] != goal[i] and board[i] != 0 else 0 for i in range(9)])
...
def main():
    ...
    solution = best_first_search(start_board, goal_board, heuristic)
